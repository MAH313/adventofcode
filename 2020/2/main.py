...
for x in range(0, len(strings)):
  ...
  params = re.match('([0-9]+)-([0-9]+)\s([a-z]):\s([a-z]+)', strings[x])

  #count = len(re.findall(params[3], params[4]))

  #if int(params[1]) <= count and int(params[2]) >= count:
  #  accepted_strings += 1

  char1 = params[4][int(params[1])-1:int(params[1])]
  char2 = params[4][int(params[2])-1:int(params[2])]

  if((char1 == params[3] or char2 == params[3]) and not
     (char1 == params[3] and char2 == params[3])):
